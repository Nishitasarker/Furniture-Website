                        ...
                        { name: "Manage Items", path: "/cart", icon: Package }
                         ...
                         return (
                            ...
                            className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all ${isActive ? "bg-orange-50 text-orange-600 font-semibold" : "text-gray-700 hover:bg-orange-50"}`}>
                              <item.icon size={18} className={isActive ? "text-orange-600" : "text-gray-400"} /> {item.name}
                            </Link>
                         );
                      })}
                    </div>
      ...
      <AnimatePresence>
        {isSidebarOpen && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed right-0 top-0 h-full w-56 bg-white z-[70] p-6 shadow-2xl">
              ...
              <div className="flex flex-col gap-3 font-bold">
                ...
                <Link href="/cart" className={getMobileLinkClass("/cart")} onClick={() => setIsSidebarOpen(false)}><ShoppingCart size={20}/>Cart <ChevronRight size={18} className="ml-auto"/></Link>
